...
for i=1:3
    errstrain_rand{i}  = Inf*ones(numel(epsilons),length(Ks),Nrand);
    scoretrain_rand{i} = 0*ones(numel(epsilons),length(Ks),Nrand);
    errstest_rand{i}  = Inf*ones(numel(epsilons),length(Ks),Nrand);
    scoretest_rand{i} = 0*ones(numel(epsilons),length(Ks),Nrand);
end
...
for myrand = 1:Nrand
    ...
    % here store L values for various epsilon and K
    Ls = cell(3,1);
    L1s = cell(3,1);
    L2s = cell(3,1);
    for i=1:3
    ...
    for idx_K=1:length(Ks)
        ...
        for idx_epsilon=1:length(epsilons)
            ...
            [C3, Gamma3, PiX3, Lambda3, it3, stats3, L_out3] = ...
                adamar_fmincon(Xtrain', Ytrain, K, epsilon, maxIters, nrand);
            PiX3 = PiX3';
            ...
            errstrain_rand{1}(idx_epsilon,idx_K,myrand) = norm(Lambda1*Gamma1 - Ytrain,'fro');
            scoretrain_rand{1}(idx_epsilon,idx_K,myrand) = stats1.f1score;
            
            Ls{2}(idx_epsilon,idx_K)  = L_out2.L;
            L1s{2}(idx_epsilon,idx_K) = L_out2.L1;
            L2s{2}(idx_epsilon,idx_K) = L_out2.L2;
            errstrain_rand{2}(idx_epsilon,idx_K,myrand) = norm(Lambda2*Gamma2 - Ytrain,'fro');
            scoretrain_rand{2}(idx_epsilon,idx_K,myrand) = stats2.f1score;

            Ls{3}(idx_epsilon,idx_K)  = L_out3.L;
            L1s{3}(idx_epsilon,idx_K) = L_out3.L1;
            L2s{3}(idx_epsilon,idx_K) = L_out3.L2;
            errstrain_rand{3}(idx_epsilon,idx_K,myrand) = norm(Lambda3*Gamma3 - Ytrain,'fro');
            scoretrain_rand{3}(idx_epsilon,idx_K,myrand) = stats3.f1score;
            
            % apply model to validation data
            Gammatest1 = compute_Gamma_kmeans(C1,Xtest);
            Ytest_est1 = Lambda1*Gammatest1;
            errstest_rand{1}(idx_epsilon,idx_K,myrand) = norm(Ytest_est1 - Ytest,'fro');
            [stats1] = statistics_multiclass(myonehotdecode(Ytest_est1,1:M), myonehotdecode(Ytest,1:M));
            scoretest_rand{1}(idx_epsilon,idx_K,myrand) = stats1.f1score;
            
            Gammatest2 = compute_Gamma_kmeans(C2,Xtest);
            Ytest_est2 = Lambda2*Gammatest2;
            errstest_rand{2}(idx_epsilon,idx_K,myrand) = norm(Ytest_est2 - Ytest,'fro');
            [stats2] = statistics_multiclass(myonehotdecode(Ytest_est2,1:M), myonehotdecode(Ytest,1:M));
            scoretest_rand{2}(idx_epsilon,idx_K,myrand) = stats2.f1score;

            Gammatest3 = compute_Gamma_kmeans(C3,Xtest);
            Ytest_est3 = Lambda3*Gammatest3;
            errstest_rand{3}(idx_epsilon,idx_K,myrand) = norm(Ytest_est3 - Ytest,'fro');
            [stats3] = statistics_multiclass(myonehotdecode(Ytest_est3,1:M), myonehotdecode(Ytest,1:M));
            scoretest_rand{3}(idx_epsilon,idx_K,myrand) = stats3.f1score;
...
%            legend('jensen','spg','spa','FontSize', legend_fs)
            ...
            hold on
            title(sprintf('K=%d', Ks(idx_K)))
            plot(epsilons, errstrain_rand{1}(:,idx_K,myrand),kmeans_lambda_linestyle(:), 'LineWidth', lw);
            plot(epsilons, errstrain_rand{2}(:,idx_K,myrand),jensen_linestyle(:), 'LineWidth', lw);
            plot(epsilons, errstrain_rand{3}(:,idx_K,myrand),spg_linestyle(:), 'LineWidth', lw);
...
if length(Ks) == 1
    ...
    colororder(newcolors)
    hold on
    plot(epsilons,errstrain{1},kmeans_lambda_linestyle(:), 'LineWidth', lw)
    plot(epsilons,errstrain{2},jensen_linestyle(:), 'LineWidth', lw)
    plot(epsilons,errstrain{3},spg_linestyle(:), 'LineWidth', lw)
    ...
    colororder(newcolors)
    hold on
    plot(epsilons,scoretrain{1},kmeans_lambda_linestyle(:), 'LineWidth', lw)
    plot(epsilons,scoretrain{2},jensen_linestyle(:), 'LineWidth', lw)
    plot(epsilons,scoretrain{3},spg_linestyle(:), 'LineWidth', lw)
    xlabel('$\epsilon$','interpreter','latex')
    ...
    colororder(newcolors)
    hold on
    plot(epsilons,errstest{1},kmeans_lambda_linestyle(:), 'LineWidth', lw)
    plot(epsilons,errstest{2},jensen_linestyle(:), 'LineWidth', lw)
    plot(epsilons,errstest{3},spg_linestyle(:), 'LineWidth', lw)
    xlabel('$\epsilon$','interpreter','latex')
    ...
    colororder(newcolors)
    hold on
    plot(epsilons,scoretest{1},kmeans_lambda_linestyle(:), 'LineWidth', lw)
    plot(epsilons,scoretest{2},jensen_linestyle(:), 'LineWidth', lw)
    plot(epsilons,scoretest{3},spg_linestyle(:), 'LineWidth', lw)
    xlabel('$\epsilon$','interpreter','latex')
...
[idx_err1,idx_err2,~] = min_in_matrix(errstest{1});
K_sol = Ks(idx_err2);
...
[stats] = statistics_multiclass(myonehotdecode(Yval_est,1:M), myonehotdecode(Yval,1:M));
